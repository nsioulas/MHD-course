...
C
     ...
     * IFAC,LA)
       IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       DIMENSION A(1000),B(1000),C(1000),D(1000),TRIGS(1000)
...
CDIR$ IVDEP
       ...
       C(JA+J)=A(IA+I)+(A(IB+I)+A(IC+I))
       D(JA+J)=B(IA+I)+(B(IB+I)+B(IC+I))
       C(JB+J)=C1*((A(IA+I)-0.5*(A(IB+I)+
     *A(IC+I)))-(SIN60*(B(IB+I)-B(IC+I))))
     *-S1*((B(IA+I)-0.5*(B(IB+I)+B(IC+I)))+(SIN60*(A(IB+I)-A(IC+I))))
       D(JB+J)=S1*((A(IA+I)-0.5*(A(IB+I)+
     *A(IC+I)))-(SIN60*(B(IB+I)-B(IC+I))))
     *+C1*((B(IA+I)-0.5*(B(IB+I)+B(IC+I)))+(SIN60*(A(IB+I)-A(IC+I))))
       C(JC+J)=C2*((A(IA+I)-0.5*(A(IB+I)+
     *A(IC+I)))+(SIN60*(B(IB+I)-B(IC+I))))
     *-S2*((B(IA+I)-0.5*(B(IB+I)+B(IC+I)))-(SIN60*(A(IB+I)-A(IC+I))))
       D(JC+J)=S2*((A(IA+I)-0.5*(A(IB+I)+
     *A(IC+I)))+(SIN60*(B(IB+I)-B(IC+I))))
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
      SUBROUTINE MILLTIME(ITOT,MESS)
      INTEGER*4 SYSTIMES,OLD_SYSTIMES,DIFFS
      COMMON/MTCOM/SYSTIMES(5),OLD_SYSTIMES(5),HANDLE
      DIMENSION DIFFS(5)
      CHARACTER*6 MESS
C
C SUBROUTINE GIVING CPU AND PAGE FAULTS ON CHANNEL 6
C ** SPECIFIC TO VAX **
C CALL INIT_MILLTIME BEFORE CALLING MILLTIME
C MESS .. 6 CHARACTER MESSAGE OUTPUT AT SAME TIME
C ITOT .. = 0 TOTALS SINCE CALL TO INIT_MILLTIME
C         = 1 VALUES SINCE LAST CALL TO MILLTIME
C
      DO I=2,5
      OLD_SYSTIMES(I)=SYSTIMES(I)
*      CALL LIB$STAT_TIMER(I,SYSTIMES(I),HANDLE)
      END DO
      IF(ITOT.EQ.1)GOTO 100
      DO I=2,5
      DIFFS(I)=SYSTIMES(I)-OLD_SYSTIMES(I)
      END DO
      CPU=DIFFS(2)/100.
      WRITE(6,1000)MESS,CPU,DIFFS(5)
 1000 FORMAT(' ',A6,' ... CPU TIME:',F8.2,'  PF:',I7)
      RETURN
 100  CPU=SYSTIMES(2)/100.0
      WRITE(6,1000)MESS,CPU,SYSTIMES(5)
      RETURN
C
      ENTRY INIT_MILLTIME
